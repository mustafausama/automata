...
class DFA:
    ...
    def to_visualDFA(self) -> VisualDFA:
        visualDFA_states = {self._frozen_to_str(state) for state in self.states}
        visualDFA_input_symbols = self.alphabet.copy()
        visualDFA_states_transitions = {}
        for key, val in self.transitions.items():
            new_key = self._frozen_to_str(key)
            visualDFA_states_transitions[new_key] = {}
            for symbol, target in val.items():
                visualDFA_states_transitions[new_key][symbol] = self._frozen_to_str(target)

        visualDFA_initial_state = self._frozen_to_str(self.start_state)
        visualDFA_accept_states = {self._frozen_to_str(state) for state in self.accept_states}
    ...
    def _frozen_to_str(self, states: frozenset[str]) -> str:
        if not states:
            return '@'
